"""Read-only adapters from existing local artifacts to project evidence evidence inputs.
...
DEFAULT_GITHUB_EVIDENCE_MEMORY_DIR = ROOT_DIR / "information" / "github_evidence_memory"
DEFAULT_PROJECT_CHANGE_MEMORY_PATH = ROOT_DIR / "information" / "project_change_memory.json"
...
class ProjectEvidenceInputSourcePaths:
    """Injectable local artifact paths; ``None`` disables that source."""

    github_evidence_memory_dir: Path | None = DEFAULT_GITHUB_EVIDENCE_MEMORY_DIR
    project_change_memory_path: Path | None = DEFAULT_PROJECT_CHANGE_MEMORY_PATH
...
def build_project_evidence_content_hash(payload: Mapping[str, Any]) -> str:
    """Build the canonical SHA-256 digest used for safe project evidence input payloads."""
...
def _read_json(path: Path, label: str) -> tuple[Any | None, list[ProjectEvidencePipelineWarning]]:
    if not path.is_file():
...
def _read_jsonl(path: Path, label: str) -> tuple[list[Any], list[ProjectEvidencePipelineWarning]]:
    if not path.is_file():
        return [], [_warning("source_file_missing", f"Optional {label} artifact was not found.")]
    records: list[Any] = []
    warnings: list[ProjectEvidencePipelineWarning] = []
    try:
...
def _validated_input(
    builder: Callable[[], ProjectEvidenceInput],
    *,
    project_id: str | None,
    source_id: str | None,
) -> tuple[ProjectEvidenceInput | None, ProjectEvidencePipelineWarning | None]:
    try:
        return builder(), None
    except (TypeError, ValueError):
        return None, _warning(
            "record_validation_failed",
            "A structured source record failed project evidence bounded-content validation.",
            project_id=project_id,
            source_id=source_id,
        )


def _github_evidence_chunk_ref(record: Mapping[str, Any], project_id: str) -> EvidenceSourceRef:
...
def _github_evidence_direct_ref(
    ...
    *,
    ...
    return EvidenceSourceRef(
        ...
        source_id=source_id,
        project_id=project_id,
        content_hash=build_project_evidence_content_hash(safe_payload),
    )


def load_github_evidence_inputs(
    memory_dir: Path,
    *,
    project_id: str | None = None,
) -> tuple[list[ProjectEvidenceInput], list[ProjectEvidencePipelineWarning]]:
    """Adapt stored GitHub evidence summaries, evidence cards, and capability facts."""

    chunk_records, warnings = _read_jsonl(memory_dir / "evidence_chunks.jsonl", "GitHub evidence evidence chunks")
    chunk_index = {
        _source_id(record, "chunk_id"): record
        for record in chunk_records
        if isinstance(record, dict) and _source_id(record, "chunk_id")
    }
    inputs: list[ProjectEvidenceInput] = []
    specifications = (
        ("raw_change_summaries.jsonl", "GitHub evidence raw change summaries", _adapt_github_evidence_change_summary),
        ("evidence_cards.jsonl", "GitHub evidence evidence cards", _adapt_github_evidence_card),
        ("capability_facts.jsonl", "GitHub evidence capability facts", _adapt_github_evidence_capability_fact),
    )
    for filename, label, adapter in specifications:
        ...
        for record in records:
            ...
            if not record_id:
                ...
                continue
            if adapter in (_adapt_github_evidence_change_summary, _adapt_github_evidence_card):
...
def _github_evidence_refs(
    ...
    for source_id in source_ids:
        ...
        ref = _github_evidence_chunk_ref(chunk, project_id)
...
def _adapt_github_evidence_change_summary(
    record: Mapping[str, Any],
    chunk_index: Mapping[str, Mapping[str, Any]],
) -> tuple[ProjectEvidenceInput | None, ProjectEvidencePipelineWarning | None]:
...
def _adapt_github_evidence_card(
    ...
    def build() -> ProjectEvidenceInput:
        return ProjectEvidenceInput(
            ...
            summary=mechanism or problem or "Structured GitHub evidence evidence card.",
...
def _adapt_github_evidence_capability_fact(
    record: Mapping[str, Any],
    ...
    def build() -> ProjectEvidenceInput:
        return ProjectEvidenceInput(
            ...
            source_refs=[_github_evidence_direct_ref(record, source_type="github_evidence_capability_fact", source_id=source_id, project_id=project, safe_payload=safe_payload)],
            content_hash=build_project_evidence_content_hash(safe_payload),
        )
    return _validated_input(build, project_id=project, source_id=source_id)


def load_project_change_inputs(
    path: Path,
    *,
    project_id: str | None = None,
) -> tuple[list[ProjectEvidenceInput], list[ProjectEvidencePipelineWarning]]:
    payload, warnings = _read_json(path, "project change memory memory")
    if payload is None:
        return [], warnings
    if not isinstance(payload, dict):
        return [], [*warnings, _warning("source_schema_invalid", "The project change memory memory root must be a JSON object.")]
    if payload.get("schema_version") != PROJECT_CHANGE_MEMORY_SCHEMA_VERSION:
        return [], [*warnings, _warning("unsupported_source_schema", "The project change memory memory schema version is unsupported.")]
    projects = payload.get("projects")
    if not isinstance(projects, dict):
        return [], [*warnings, _warning("source_schema_invalid", "The project change memory projects field must be a JSON object.")]
    inputs: list[ProjectEvidenceInput] = []
    for project_key in sorted(projects, key=str):
        ...
        if not isinstance(entry, dict):
            warnings.append(_warning("record_not_object", "A project change memory project entry is not a JSON object.", project_id=parent_project or None))
            continue
        entry_project = _normalize_project_id(entry.get("project_id")) or parent_project
        if not entry_project:
            warnings.append(_warning("missing_project_id", "A project change memory project entry has no project ID."))
            continue
        if parent_project and entry_project != parent_project:
            warnings.append(_warning("project_id_mismatch", "A project change memory project entry does not match its project key.", project_id=entry_project))
            continue
        if project_id is not None and entry_project != project_id:
            continue
        for field_name, source_type, id_field, adapter in (
            ("raw_change_summaries", "project_change_raw_change_summary", "change_id", _adapt_project_change_change),
            ("evidence_cards", "project_change_evidence_card", "evidence_id", _adapt_project_change_card),
            ("capability_facts", "project_change_capability_fact", "capability_id", _adapt_project_change_capability),
        ):
            records = entry.get(field_name, [])
            if not isinstance(records, list):
                warnings.append(_warning("source_schema_invalid", f"A project change memory {field_name} collection is not a list.", project_id=entry_project))
                continue
            for record in records:
                if not isinstance(record, dict):
                    warnings.append(_warning("record_not_object", f"A project change memory {field_name} record is not a JSON object.", project_id=entry_project))
                    continue
                record_id = _source_id(record, id_field)
                record_project = _normalize_project_id(record.get("project_id"))
                if not record_project:
                    warnings.append(_warning("missing_project_id", f"A project change memory {field_name} record has no project ID.", source_id=record_id or None))
                    continue
                if record_project != entry_project:
                    warnings.append(_warning("project_id_mismatch", f"A project change memory {field_name} record has a mismatched project ID.", project_id=record_project, source_id=record_id or None))
                    continue
                if not record_id:
                    warnings.append(_warning("missing_source_id", f"A project change memory {field_name} record has no stable source ID.", project_id=record_project))
                    continue
...
def _project_change_ref(record: Mapping[str, Any], source_type: str, source_id: str, project: str, safe_payload: Mapping[str, Any]) -> EvidenceSourceRef:
    return EvidenceSourceRef(
        source_type=source_type,
        source_id=source_id,
        project_id=project,
...
def _adapt_project_change_change(record: Mapping[str, Any], source_type: str, source_id: str, project: str) -> tuple[ProjectEvidenceInput | None, ProjectEvidencePipelineWarning | None]:
    summary = _text(record.get("what_changed"))
    ...
    return _validated_input(lambda: ProjectEvidenceInput(project_id=project, input_type=source_type, title=summary or f"Structured change {source_id}", summary=summary or "Structured project change memory change summary.", mechanism_signals=change_types, implementation_signals=[*([safe_payload["file_path"]] if safe_payload["file_path"] else []), *symbols], technical_tags=change_types, source_refs=[_project_change_ref(record, source_type, source_id, project, safe_payload)], content_hash=build_project_evidence_content_hash(safe_payload)), project_id=project, source_id=source_id)


def _adapt_project_change_card(record: Mapping[str, Any], source_type: str, source_id: str, project: str) -> tuple[ProjectEvidenceInput | None, ProjectEvidencePipelineWarning | None]:
    ...
    return _validated_input(lambda: ProjectEvidenceInput(project_id=project, input_type=source_type, title=_text(record.get("resume_angle")) or mechanism or f"Evidence {source_id}", summary=mechanism or problem or "Structured project change memory evidence card.", problem_signal=problem or None, mechanism_signals=[mechanism] if mechanism else [], implementation_signals=implementation, impact_signals=[*([impact] if impact else []), *claims], source_refs=[_project_change_ref(record, source_type, source_id, project, safe_payload)], content_hash=build_project_evidence_content_hash(safe_payload)), project_id=project, source_id=source_id)


def _adapt_project_change_capability(record: Mapping[str, Any], source_type: str, source_id: str, project: str) -> tuple[ProjectEvidenceInput | None, ProjectEvidencePipelineWarning | None]:
...
def load_project_memory_inputs(
    ...
    *,
    project_id: str | None = None,
) -> tuple[list[ProjectEvidenceInput], list[ProjectEvidencePipelineWarning], dict[str, str]]:
    ...
    for record in project_records:
        ...
        source_id = build_project_evidence_stable_id("esr_", current_project, {"source_type": "project_memory", **safe_payload})

        def build() -> ProjectEvidenceInput:
            return ProjectEvidenceInput(project_id=current_project, input_type="project_memory", title=project_name or current_project, summary=summary or f"Structured project memory for {project_name or current_project}.", problem_signal=problem or None, mechanism_signals=workflows, implementation_signals=[*features, *modules], impact_signals=impacts, technical_tags=tags, source_refs=[EvidenceSourceRef(source_type="project_memory", source_id=source_id, project_id=current_project, content_hash=build_project_evidence_content_hash(safe_payload), metadata={"schema_version": PROJECT_MEMORY_VERSION})], content_hash=build_project_evidence_content_hash(safe_payload))
...
def load_compact_fact_inputs(
    ...
    *,
    ...
    for entry_key in sorted(payload, key=str):
        ...
        digest = _valid_sha256(record.get("source_hash")) or build_project_evidence_content_hash(safe_payload)

        def build() -> ProjectEvidenceInput:
            return ProjectEvidenceInput(project_id=current_project, input_type="project_compact_facts", title=_text(facts.get("projectName")) or project_name or current_project, summary=summary or f"Structured compact facts for {project_name or current_project}.", implementation_signals=modules, impact_signals=impacts, technical_tags=tags, source_refs=[EvidenceSourceRef(source_type="project_compact_facts", source_id=source_id, project_id=current_project, repo=_text(record.get("repo_name")) or None, content_hash=digest)], content_hash=build_project_evidence_content_hash(safe_payload))
        result, current_warning = _validated_input(build, project_id=current_project, source_id=source_id)
        if result is not None:
            inputs.append(result)
        if current_warning is not None:
...
def _input_sort_key(item: ProjectEvidenceInput) -> tuple[str, str, str, str, str]:
...
def _warning_sort_key(item: ProjectEvidencePipelineWarning) -> tuple[str, str, str, str, str]:
    return (item.project_id or "", item.code, item.source_id or "", item.severity.value, item.message)


def load_project_evidence_inputs(
    project_id: str | None = None,
    *,
    source_paths: ProjectEvidenceInputSourcePaths | None = None,
) -> tuple[list[ProjectEvidenceInput], list[ProjectEvidencePipelineWarning]]:
    """Load all enabled local sources without writing files or merging evidence."""

    paths = source_paths or ProjectEvidenceInputSourcePaths()
    ...
    if paths.github_evidence_memory_dir is not None:
        current, current_warnings = load_github_evidence_inputs(Path(paths.github_evidence_memory_dir), project_id=requested_project)
        inputs.extend(current); warnings.extend(current_warnings)
    if paths.project_change_memory_path is not None:
        current, current_warnings = load_project_change_inputs(Path(paths.project_change_memory_path), project_id=requested_project)
        inputs.extend(current); warnings.extend(current_warnings)
...
__all__ = [
    ...
    "build_project_evidence_content_hash",
    "load_compact_fact_inputs",
    "load_github_evidence_inputs",
    "load_project_change_inputs",
