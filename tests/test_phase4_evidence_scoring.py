...
from backend.phase4_evidence_scoring import (
    DIMENSION_MAXIMUMS,
...
def test_concrete_mechanism_scores_higher_than_generic():
    concrete = evaluate_phase4_evidence_quality(fact())
    generic = evaluate_phase4_evidence_quality(fact(mechanism="Improved the pipeline"))
...
def test_concrete_implementation_scores_higher_than_generic():
    concrete = evaluate_phase4_evidence_quality(fact())
    generic = evaluate_phase4_evidence_quality(fact(implementation=["Developed features"]))
...
def test_direct_provenance_scores_higher_than_project_context():
    direct = evaluate_phase4_evidence_quality(fact(source_refs=[source()]))
    contextual_ref = source(source_type="project_memory", file_path=None, symbol=None)
    contextual = evaluate_phase4_evidence_quality(fact(source_refs=[contextual_ref]))
...
def test_explicit_safe_impact_scores_higher_than_absent_impact():
    present = evaluate_phase4_evidence_quality(fact())
    absent = evaluate_phase4_evidence_quality(fact(safe_impact=[]))
...
def test_absent_impact_does_not_prevent_high_value_score():
    evaluation = evaluate_phase4_evidence_quality(fact(safe_impact=[]))
    assert evaluation.quality_band == "high_value"


def test_absent_problem_does_not_prevent_high_value_score():
    evaluation = evaluate_phase4_evidence_quality(fact(problem=""))
    assert evaluation.quality_band == "high_value"


def test_coherent_technical_signals_beat_unrelated_tag_quantity():
    coherent = evaluate_phase4_evidence_quality(fact())
    unrelated = evaluate_phase4_evidence_quality(
        fact(
...
def test_empty_mechanism_is_a_blocker():
    ...
    evaluation = evaluate_phase4_evidence_quality(item)
...
def test_empty_implementation_materially_lowers_score():
    complete = evaluate_phase4_evidence_quality(fact())
    incomplete = evaluate_phase4_evidence_quality(fact(implementation=[], status=EvidenceStatus.WEAK))
...
def test_unknown_evidence_type_can_still_score_highly():
    evaluation = evaluate_phase4_evidence_quality(fact(evidence_type=EvidenceType.UNKNOWN))
    assert evaluation.quality_band == "high_value"
...
def test_generic_mechanism_receives_penalty(generic):
    evaluation = evaluate_phase4_evidence_quality(fact(mechanism=generic))
...
def test_generic_impact_receives_limited_credit():
    evaluation = evaluate_phase4_evidence_quality(fact(safe_impact=["Improved reliability"]))
    assert evaluation.breakdown["safe_impact_quality"] <= 2


def test_generic_phrase_plus_concrete_mechanism_is_not_rejected():
    evaluation = evaluate_phase4_evidence_quality(
...
def test_short_technical_identifier_is_not_generic():
    evaluation = evaluate_phase4_evidence_quality(fact(mechanism="SHA256"))
...
def test_unsupported_claims_receive_penalty(claim):
    evaluation = evaluate_phase4_evidence_quality(fact(safe_impact=[claim]))
...
def test_technical_numbers_do_not_trigger_metric_penalty(technical_value):
    evaluation = evaluate_phase4_evidence_quality(fact(implementation=[technical_value]))
...
def test_approximate_metric_requires_approximate_language():
    supported = evaluate_phase4_evidence_quality(
        fact(safe_impact=["Approximately 20% reduction"], metric_support=MetricSupport.APPROXIMATE)
    )
    unsupported = evaluate_phase4_evidence_quality(
...
def test_metric_none_does_not_block_non_numeric_fact():
    evaluation = evaluate_phase4_evidence_quality(fact(metric_support=MetricSupport.NONE))
    assert not evaluation.unsupported_claim_penalty


def test_accepted_high_quality_fact_remains_accepted():
    scored = score_phase4_evidence_fact(fact(status=EvidenceStatus.ACCEPTED))
    assert scored.status is EvidenceStatus.ACCEPTED


def test_supporting_high_quality_fact_is_not_upgraded():
    scored = score_phase4_evidence_fact(fact(status=EvidenceStatus.SUPPORTING))
    assert scored.status is EvidenceStatus.SUPPORTING


def test_weak_high_score_fact_is_not_upgraded():
    scored = score_phase4_evidence_fact(fact(status=EvidenceStatus.WEAK))
...
def test_hard_blocker_may_downgrade_accepted_fact():
    scored = score_phase4_evidence_fact(
...
def test_confidence_is_not_rewritten_or_derived_from_score():
    ...
    scored = score_phase4_evidence_fact(original)
...
def test_breakdown_sums_and_contains_required_dimensions():
    scored = score_phase4_evidence_fact(fact())
...
def test_penalties_and_quality_band_are_deterministic():
    scored = score_phase4_evidence_fact(fact(implementation=[] , status=EvidenceStatus.WEAK))
...
def test_score_is_integer_and_model_range_is_zero_to_one_hundred():
    ...
    Phase4EvidenceFact.from_dict({**scored.to_dict(), "quality_score": 100})
    with pytest.raises(ValueError, match="between 0 and 100"):
        Phase4EvidenceFact.from_dict({**scored.to_dict(), "quality_score": 101})
...
def test_scoring_does_not_mutate_input():
    ...
    score_phase4_evidence_fact(original)
    assert original.to_json() == before


def test_equivalent_facts_have_identical_score_and_breakdown():
    one = score_phase4_evidence_fact(fact())
    two = score_phase4_evidence_fact(fact())
...
def test_input_order_does_not_affect_scores_or_output_order():
    ...
    first, first_report = score_phase4_evidence_facts([zeta, alpha])
    second, second_report = score_phase4_evidence_facts([alpha, zeta])
...
def test_bounded_decision_sample_is_input_order_independent():
    ...
    _, first_report = score_phase4_evidence_facts(items)
    _, second_report = score_phase4_evidence_facts(reversed(items))
    assert len(first_report.decisions) == 100
    assert first_report.decisions == second_report.decisions


def test_grouped_structural_status_preserves_step4_status_on_downgrade():
    item = fact(
        safe_impact=["Guaranteed reliability"],
        status=EvidenceStatus.ACCEPTED,
    )
    _, report = score_phase4_evidence_facts([item])
...
def test_repeated_scoring_is_idempotent():
    once = score_phase4_evidence_fact(fact())
    twice = score_phase4_evidence_fact(once)
...
def test_machine_paths_timestamps_and_tag_order_do_not_affect_score():
    ...
    one = evaluate_phase4_evidence_quality(fact(source_refs=[one_ref], technical_tags=["Python", "validation"]))
    two = evaluate_phase4_evidence_quality(fact(source_refs=[two_ref], technical_tags=["validation", "Python"]))
...
def test_implementation_order_is_preserved_and_scoring_is_stable():
    ...
    first_scored = score_phase4_evidence_fact(first)
    second_scored = score_phase4_evidence_fact(second)
...
def test_raw_change_direct_provenance_scores_conservatively():
    evaluation = evaluate_phase4_evidence_quality(
        fact(source_refs=[source(source_type="phase2_raw_change_summary")])
    )
    assert evaluation.breakdown["provenance_strength"] == 11


def test_capability_weak_fact_remains_weak():
    capability_ref = source(source_type="phase2_capability_fact", file_path=None, symbol=None)
    scored = score_phase4_evidence_fact(
...
def test_duplicate_refs_do_not_inflate_provenance():
    ref = source()
    one = evaluate_phase4_evidence_quality(fact(source_refs=[ref]))
    duplicated = evaluate_phase4_evidence_quality(fact(source_refs=[ref, Phase4SourceRef.from_dict(ref.to_dict())]))
...
def test_multiple_independent_direct_refs_may_improve_provenance():
    one = evaluate_phase4_evidence_quality(fact(source_refs=[source("one")]))
    two = evaluate_phase4_evidence_quality(
...
def test_oversized_invalid_fact_is_not_truncated():
    ...
    with pytest.raises(ValueError, match="maximum length"):
        score_phase4_evidence_fact(item)
...
def test_no_files_are_written(tmp_path):
    ...
    score_phase4_evidence_facts([fact()])
...
def test_import_has_no_external_or_runtime_side_effects():
    module_name = "backend.phase4_evidence_scoring"
    sys.modules.pop(module_name, None)
    before = set(sys.modules)
    importlib.import_module(module_name)
    newly_loaded = set(sys.modules) - before
    assert "backend.api_server" not in newly_loaded
    assert "backend.phase3_pipeline" not in newly_loaded
...
def test_real_weak_and_project_context_are_not_promoted():
    ...
    assert all(item.status is EvidenceStatus.WEAK for item in scored if item.source_refs[0].source_type == "phase2_capability_fact")
