...
from backend.phase4_evidence_normalizer import (
...
from backend.phase4_models import Phase4EvidenceInput, Phase4SourceRef
...
def test_bounded_text_normalization_preserves_punctuation_and_case():
    ...
    normalized = normalize_phase4_input(item)
...
def test_project_id_and_input_type_preserve_canonical_case():
    ...
    object.__setattr__(item, "input_type", " phase2_evidence_card ")
    object.__setattr__(item.source_refs[0], "project_id", "WorkAgent")
    normalized = normalize_phase4_input(item)
    assert normalized.project_id == "WorkAgent"
    assert normalized.input_type == "phase2_evidence_card"


def test_file_path_uses_existing_model_normalization():
    normalized = normalize_phase4_input(evidence(source_refs=[source(file_path=r"backend\phase4\item.py")]))
    assert normalized.source_refs[0].file_path == "backend/phase4/item.py"
...
def test_source_ref_order_preserved_and_exact_duplicates_removed():
    ...
    item.source_refs.append(Phase4SourceRef.from_dict(second.to_dict()))
    normalized = normalize_phase4_input(item)
...
def test_source_refs_with_partial_identity_match_remain_separate():
    ...
    normalized = normalize_phase4_input(evidence(source_refs=[first, second]))
...
def test_input_object_is_not_mutated():
    ...
    before = item.to_json()
    normalize_phase4_input(item)
    assert item.to_json() == before


def test_normalization_is_idempotent_and_serially_stable():
    once = normalize_phase4_input(evidence())
    twice = normalize_phase4_input(once)
...
def test_equivalent_normalized_records_have_same_id():
    ...
    assert normalize_phase4_input(one).input_id == normalize_phase4_input(two).input_id
...
def test_timestamps_and_absolute_machine_paths_do_not_enter_id():
    ...
    one = normalize_phase4_input(evidence(source_refs=[one_ref]))
    two = normalize_phase4_input(evidence(source_refs=[two_ref]))
...
def test_content_hash_is_recomputed_after_normalization():
    ...
    changed = normalize_phase4_input(evidence(summary="Changed."))
...
def test_normalized_collection_order_is_deterministic():
    ...
    assert [item.project_id for item in normalize_phase4_inputs([zeta, alpha])] == ["alpha", "zeta"]


def test_identical_records_collapse_as_exact_duplicate():
    item = evidence()
    output, report = dedupe_phase4_inputs([item, Phase4EvidenceInput.from_dict(item.to_dict())])
    assert len(output) == 1
...
def test_repeated_traversal_with_stale_id_collapses():
    one = evidence()
    two = evidence(input_id="p4in_stale_traversal_copy")
    output, report = dedupe_phase4_inputs([one, two])
    assert len(output) == 1
...
def test_duplicates_across_projects_remain_separate():
    ...
    output, _ = dedupe_phase4_inputs([one, two])
    assert len(output) == 2
...
def test_similar_summary_from_different_source_types_remains_separate():
    ...
    two = evidence(input_type="phase2_evidence_card")
    output, report = dedupe_phase4_inputs([one, two])
    assert len(output) == 2
...
def test_card_and_capability_and_project_memory_remain_separate():
    ...
    capability = evidence(input_type="phase2_capability_fact", source_refs=[source(source_type="phase2_capability_fact")])
    memory = evidence(input_type="project_memory", source_refs=[source(source_type="project_memory")])
    output, _ = dedupe_phase4_inputs([card, capability, memory])
    assert len(output) == 3
...
def test_deterministic_winner_and_output_do_not_depend_on_input_order():
    original = evidence()
    stale = evidence(input_id="p4in_stale_copy")
    alpha = evidence(project_id="alpha", source_refs=[source(project_id="alpha")])
    forward, forward_report = dedupe_phase4_inputs([stale, alpha, original])
    reverse, reverse_report = dedupe_phase4_inputs([original, alpha, stale])
...
def test_report_counters_include_in_record_duplicates():
    ...
    output, report = normalize_and_dedupe_phase4_inputs([item, item])
    assert len(output) == 1
...
def test_decisions_are_bounded_and_contain_no_evidence_text():
    ...
    _, report = dedupe_phase4_inputs([item, item])
...
def test_same_declared_input_id_with_different_payload_is_detected_safely():
    ...
    with pytest.raises(Phase4IntegrityError, match="same_input_id_different_payload") as captured:
        dedupe_phase4_inputs([one, two])
...
def test_same_declared_hash_same_source_with_different_payload_is_detected():
    ...
    with pytest.raises(Phase4IntegrityError, match="same_content_hash_different_payload"):
        dedupe_phase4_inputs([one, two])
...
def test_import_has_no_runtime_or_external_side_effects():
    module_name = "backend.phase4_evidence_normalizer"
...
def test_real_step2_output_passes_read_only_through_step3():
    ...
    output, report = dedupe_phase4_inputs(inputs)
