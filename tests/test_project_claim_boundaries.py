...
import backend.project_claim_boundaries as claim_boundary_module
...
def allowed(boundary: ProjectClaimBoundary | None, prefix: str) -> list[str]:
    assert boundary is not None
    return [value for value in boundary.allowed_claims if value.startswith(prefix + ":")]


def metadata_note(boundary: ProjectClaimBoundary, prefix: str) -> str:
...
def test_high_quality_direct_fact_creates_mechanism_claim(quality):
    boundary = build_project_evidence_claim_boundary(fact(quality_score=quality))
...
def test_low_quality_direct_fact_creates_no_affirmative_claim(quality):
    assert build_project_evidence_claim_boundary(fact(quality_score=quality)) is None
...
def test_weak_or_rejected_fact_creates_no_affirmative_claim(status):
    assert build_project_evidence_claim_boundary(fact(status=status)) is None
...
def test_capability_context_weak_fact_creates_no_allowed_claim():
    item = fact(source_type="github_evidence_capability_fact", status=EvidenceStatus.WEAK)
    assert build_project_evidence_claim_boundary(item) is None
...
def test_missing_problem_is_not_inferred_from_mechanism():
    boundary = build_project_evidence_claim_boundary(fact(problem=""))
...
def test_generic_mechanism_is_blocked(mechanism):
    assert build_project_evidence_claim_boundary(fact(mechanism, problem="")) is None


def test_mechanism_does_not_create_impact_or_capability_claim():
    boundary = build_project_evidence_claim_boundary(fact("deterministic ordering", problem=""))
    assert allowed(boundary, "mechanism")
...
def test_explicit_implementation_is_safely_normalized(implementation, expected):
    boundary = build_project_evidence_claim_boundary(fact(implementation=[implementation]))
...
def test_implementation_claim_retains_evidence_support_id():
    item = fact(evidence_fact_id="pef_explicit_support")
    boundary = build_project_evidence_claim_boundary(item)
...
def test_exact_duplicate_implementation_claims_merge_support_ids():
    ...
    boundary = build_project_claim_boundary("workagent", [first, second])
...
def test_explicit_safe_impact_creates_bounded_impact_only():
    boundary = build_project_evidence_claim_boundary(fact(safe_impact=["Kept evidence ordering stable."]))
...
def test_unsafe_impact_is_blocked_without_removing_mechanism(impact, policy):
    boundary = build_project_evidence_claim_boundary(fact(safe_impact=[impact]))
    assert not allowed(boundary, "impact")
    assert allowed(boundary, "mechanism")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("github_evidence_capability_fact", EvidenceStatus.WEAK),
    ],
)
...
def test_technology_cannot_leak_between_projects():
    ...
    boundaries, _ = build_project_claim_boundaries_by_project([first, second])
...
def test_builder_does_not_read_jd_resume_or_global_ontology(monkeypatch):
    ...
    boundary = build_project_evidence_claim_boundary(fact(technical_tags=["Python"]))
...
def test_present_capability_fact_creates_canonical_capability_claim_and_taxonomy_policies():
    ...
    boundary = build_project_capability_claim_boundary(cap, evidence_facts_by_id={evidence.evidence_fact_id: evidence})
...
def test_absent_capability_fact_creates_no_boundary():
    evidence = fact()
    assert build_project_capability_claim_boundary(
...
def test_missing_or_cross_project_capability_evidence_blocks_claim():
    evidence = fact()
    cap = capability(evidence)
    assert build_project_capability_claim_boundary(cap, evidence_facts_by_id={}) is None
...
def test_zero_capabilities_still_produces_project_boundary():
    boundary = build_project_claim_boundary("workagent", [fact()], [])
    assert boundary is not None
...
def test_explicit_metric_support_creates_metric_claim():
    boundary = build_project_evidence_claim_boundary(
...
def test_metric_support_none_blocks_numeric_impact():
    boundary = build_project_evidence_claim_boundary(fact(safe_impact=["Reduced processing time by 25%"]))
    assert not allowed(boundary, "metric")
    assert not allowed(boundary, "impact")
    assert "unsupported_metric" in boundary.forbidden_claims
...
def test_conflicting_metric_support_resolves_conservatively():
    ...
    boundary = build_project_claim_boundary("workagent", [explicit, unsupported])
    assert not allowed(boundary, "metric")
    assert not allowed(boundary, "impact")
...
def test_forbidden_claims_are_deterministic_and_deduplicated():
    first = build_project_evidence_claim_boundary(fact())
    second = build_project_evidence_claim_boundary(fact())
...
def test_same_project_aggregates_and_different_projects_remain_exactly_separate():
    ...
    boundaries, report = build_project_claim_boundaries_by_project(reversed(records))
...
def test_project_boundary_contains_only_support_from_its_project():
    ...
    boundaries, _ = build_project_claim_boundaries_by_project([alpha, beta])
...
def test_empty_or_nonqualifying_project_has_documented_empty_result():
    assert build_project_claim_boundary("workagent", []) is None
    assert build_project_claim_boundary("workagent", [fact(status=EvidenceStatus.WEAK)]) is None
...
def test_duplicate_support_does_not_raise_claim_confidence():
    ...
    boundary = build_project_claim_boundary("workagent", [medium, high])
    assert "|high|" in metadata_note(boundary, "mechanism")
    assert build_project_claim_boundary("workagent", [high]).boundary_id != ""
...
def test_legacy_label_does_not_inflate_confidence_or_become_allowed():
    ...
    boundary = build_project_claim_boundary("workagent", [direct, legacy])
    assert "|medium|" in metadata_note(boundary, "mechanism")
    assert not allowed(boundary, "capability")
...
def test_structured_claim_registry_is_immutable_and_validated():
    assert list_project_claim_types() == CLAIM_TYPES
...
def test_validation_rejects_invalid_or_unsafe_claims(claim, error):
    ...
    result = validate_project_claim_boundary(boundary)
...
def test_validation_detects_unknown_evidence_and_capability_ids():
    ...
    cap = capability(evidence)
    boundary = build_project_capability_claim_boundary(cap, evidence_facts_by_id={evidence.evidence_fact_id: evidence})
    result = validate_project_claim_boundary(boundary, evidence_facts_by_id={}, capability_facts_by_id={})
...
def test_inputs_are_not_mutated():
    ...
    before = item.to_json()
    build_project_evidence_claim_boundary(item)
    build_project_claim_boundary(item.project_id, [item])
...
def test_equivalent_evidence_is_idempotent_and_material_claim_changes_change_id():
    first = build_project_evidence_claim_boundary(fact())
    second = build_project_evidence_claim_boundary(fact())
    changed_allowed = build_project_evidence_claim_boundary(fact(mechanism="canonical serialization"))
    changed_forbidden = build_project_evidence_claim_boundary(
...
def test_technical_tag_order_and_input_order_do_not_change_results():
    ...
    assert build_project_evidence_claim_boundary(first) == build_project_evidence_claim_boundary(reordered)
    second = fact(source_id="b", mechanism="canonical serialization")
    forward = build_project_claim_boundaries_by_project([first, second])
    reverse = build_project_claim_boundaries_by_project([second, first])
...
def test_bounded_decision_sampling_and_truncation_are_deterministic():
    ...
    forward_boundary, forward_report = build_project_claim_boundaries_by_project(records)
    reverse_boundary, reverse_report = build_project_claim_boundaries_by_project(reversed(records))
...
def test_combined_claim_type_limits_never_exceed_model_list_bound():
    claims = [
        ProjectStructuredClaim(
            ...
            evidence_fact_ids=(f"pef_{claim_type}_{index}",),
            capability_fact_ids=(),
...
def test_machine_specific_absolute_prefix_does_not_change_boundary_id():
    first = fact(
        ...
        evidence_fact_id="pef_same_safe_source",
    )
    second = fact(
        implementation=["D:/Users/bob/work/backend/module.py::build_result"],
        evidence_fact_id="pef_same_safe_source",
    )
    assert build_project_evidence_claim_boundary(first) == build_project_evidence_claim_boundary(second)


def test_same_fact_id_different_payload_is_conflicted_and_removed():
    first = fact(evidence_fact_id="pef_conflict", mechanism="deterministic ordering")
    second = fact(evidence_fact_id="pef_conflict", mechanism="canonical serialization")
    boundaries, report = build_project_claim_boundaries_by_project([first, second])
    assert boundaries == {}
...
def test_no_files_are_written(tmp_path):
    ...
    build_project_claim_boundaries_by_project([fact()])
    assert list(tmp_path.rglob("*")) == before


def test_import_has_no_file_network_database_or_model_side_effects(monkeypatch):
    module_name = "backend.project_claim_boundaries"
...
def test_step5_scored_facts_and_step7_zero_capabilities_pass_directly():
    ...
    boundaries, report = build_project_claim_boundaries_by_project(scored, capabilities)
    ...
    assert all(isinstance(value, ProjectClaimBoundary) for value in boundaries.values())
    assert all(
        validate_project_claim_boundary(value, evidence_facts_by_id={item.evidence_fact_id: item for item in scored}).valid
...
def test_real_data_audit_is_read_only_and_weak_context_never_creates_allowed_claims():
    ...
    boundaries, report = build_project_claim_boundaries_by_project(scored)
    weak_boundaries = [build_project_evidence_claim_boundary(item) for item in weak]
...
def test_project_subject_enum_is_backward_compatible_extension():
    ...
    assert set(GLOBAL_FORBIDDEN_POLICIES) <= set(build_project_evidence_claim_boundary(fact()).forbidden_claims)
