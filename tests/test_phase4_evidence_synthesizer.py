...
from backend.phase4_evidence_synthesizer import (
...
def test_valid_evidence_card_produces_accepted_fact():
    fact = synthesize_phase4_evidence_fact(evidence())
...
def test_valid_raw_change_produces_accepted_fact_and_explicit_type():
    ...
    fact = synthesize_phase4_evidence_fact(item)
    assert fact is not None
    assert fact.status is EvidenceStatus.ACCEPTED
...
def test_concrete_project_memory_is_at_most_supporting():
    fact = synthesize_phase4_evidence_fact(evidence(input_type="project_memory"))
    assert fact is not None
    assert fact.status is EvidenceStatus.SUPPORTING
    assert fact.confidence is Confidence.LOW
...
def test_generic_project_memory_positioning_is_rejected(generic):
    ...
    assert synthesize_phase4_evidence_fact(item) is None


def test_missing_problem_is_allowed_for_valid_feature():
    fact = synthesize_phase4_evidence_fact(evidence(problem_signal=None))
    assert fact is not None
    assert fact.problem == ""


def test_missing_safe_impact_is_allowed():
    fact = synthesize_phase4_evidence_fact(evidence(impact_signals=[]))
    assert fact is not None
    assert fact.safe_impact == []


def test_missing_mechanism_rejects_candidate():
    assert synthesize_phase4_evidence_fact(evidence(mechanism_signals=[])) is None


def test_missing_implementation_is_weak_not_upgraded():
    ref = source(file_path=None, symbol=None)
    fact = synthesize_phase4_evidence_fact(evidence(implementation_signals=[], source_refs=[ref]))
    assert fact is not None
...
def test_generic_implementation_does_not_satisfy_structural_acceptance():
    ref = source(file_path=None, symbol=None)
    fact = synthesize_phase4_evidence_fact(
        evidence(
...
def test_fields_are_copied_only_from_explicit_signals_in_order():
    ...
    item = evidence(
        ...
        source_refs=[Phase4SourceRef.from_dict({**ref.to_dict(), "project_id": "WorkAgent"}) for ref in refs],
    )
    fact = synthesize_phase4_evidence_fact(item)
    assert fact is not None
...
def test_duplicate_source_refs_removed_only_when_fully_equal():
    ...
    item.source_refs.append(Phase4SourceRef.from_dict(item.source_refs[0].to_dict()))
    fact = synthesize_phase4_evidence_fact(item)
    assert fact is not None
...
def test_technical_mechanism_does_not_infer_impact(mechanism, forbidden_impact):
    fact = synthesize_phase4_evidence_fact(evidence(mechanism_signals=[mechanism], impact_signals=[]))
    assert fact is not None
...
def test_numbers_do_not_infer_metrics(mechanism, implementation):
    fact = synthesize_phase4_evidence_fact(
        evidence(input_type="phase2_raw_change_summary", mechanism_signals=[mechanism], implementation_signals=implementation)
    )
    assert fact is not None
    assert fact.metric_support is MetricSupport.NONE
...
def test_exact_provenance_card_and_change_bundle_safely():
    card = evidence(input_type="phase2_evidence_card")
    change = evidence(
        input_type="phase2_raw_change_summary",
        problem_signal=None,
        mechanism_signals=["validation_logic_update"],
    ...
    bundles = build_phase4_evidence_bundles([change, card])
    facts, report = synthesize_phase4_evidence_facts([change, card])
...
def test_similar_wording_without_exact_provenance_remains_separate():
    ...
    facts, _ = synthesize_phase4_evidence_facts([one, two])
    assert len(facts) == 2
...
def test_distinct_project_or_provenance_never_bundles(first_ref, second_ref):
    ...
    assert len(build_phase4_evidence_bundles([first, second])) == 2


def test_project_memory_does_not_bundle_with_code_evidence():
    memory = evidence(input_type="project_memory")
    card = evidence(input_type="phase2_evidence_card")
    assert len(build_phase4_evidence_bundles([memory, card])) == 2
...
def test_conflicting_exact_provenance_inputs_are_retained_separately():
    ...
    facts, report = synthesize_phase4_evidence_facts([first, second])
    assert len(facts) == 2
...
def test_direct_explicit_high_confidence_may_be_preserved():
    ...
    fact = synthesize_phase4_evidence_fact(evidence(source_refs=[ref]))
    assert fact is not None
...
def test_repetition_and_tag_count_do_not_promote_confidence():
    ...
    facts, _ = synthesize_phase4_evidence_facts([item, item])
    assert all(fact.confidence is Confidence.LOW for fact in facts)


def test_status_is_structural_not_score_based():
    fact = synthesize_phase4_evidence_fact(evidence(impact_signals=[], technical_tags=[]))
    assert fact is not None
    assert fact.status is EvidenceStatus.ACCEPTED
    assert fact.quality_score is None
...
def test_unsupported_numeric_or_absolute_impact_is_dropped_without_losing_fact(unsafe):
    facts, report = synthesize_phase4_evidence_facts([evidence(impact_signals=[unsafe])])
    assert len(facts) == 1
...
def test_inputs_are_not_mutated():
    item = evidence()
    before = item.to_json()
    synthesize_phase4_evidence_facts([item])
...
def test_no_files_are_written(tmp_path):
    ...
    synthesize_phase4_evidence_facts([evidence()])
...
def test_import_has_no_external_or_runtime_side_effects():
    module_name = "backend.phase4_evidence_synthesizer"
...
def test_equivalent_input_produces_same_fact_id_and_repeated_output():
    one = synthesize_phase4_evidence_fact(evidence())
    two = synthesize_phase4_evidence_fact(evidence())
...
def test_materially_different_mechanism_changes_fact_id():
    one = synthesize_phase4_evidence_fact(evidence(mechanism_signals=["mechanism one"]))
    two = synthesize_phase4_evidence_fact(evidence(mechanism_signals=["mechanism two"]))
    assert one is not None and two is not None
    assert one.evidence_fact_id != two.evidence_fact_id


def test_implementation_order_changes_fact_id():
    one = synthesize_phase4_evidence_fact(evidence(implementation_signals=["read", "validate"]))
    two = synthesize_phase4_evidence_fact(evidence(implementation_signals=["validate", "read"]))
    assert one is not None and two is not None
    assert one.evidence_fact_id != two.evidence_fact_id


def test_tag_order_does_not_change_fact_id():
    one = synthesize_phase4_evidence_fact(evidence(technical_tags=["Python", "validation"]))
    two = synthesize_phase4_evidence_fact(evidence(technical_tags=["validation", "Python"]))
    assert one is not None and two is not None
    assert one.evidence_fact_id == two.evidence_fact_id
...
def test_output_order_and_report_are_deterministic():
    ...
    first, first_report = synthesize_phase4_evidence_facts([zeta, alpha])
    second, second_report = synthesize_phase4_evidence_facts([alpha, zeta])
...
def test_report_counters_are_structural():
    card = evidence()
    change = evidence(input_type="phase2_raw_change_summary", problem_signal=None, mechanism_signals=["validation_logic_update"], impact_signals=[])
    capability = evidence(input_type="phase2_capability_fact", implementation_signals=[], source_refs=[source(file_path=None, symbol=None)])
    rejected = evidence(input_type="project_memory", mechanism_signals=["Improved the pipeline"])
    facts, report = synthesize_phase4_evidence_facts([card, change, capability, rejected])
    assert len(facts) == 2
...
def test_real_step2_step3_output_passes_step4_read_only():
    ...
    facts, report = synthesize_phase4_evidence_facts(normalized)
